...
(function () {
        ...
        a.addCommand(b.name, {
            exec: function (a, d) {
                ...
                if (f && f.name == b.name)f.edit(); else if (b.insert)b.insert(); else if (b.template) {
                    var f = "function" == typeof b.defaults ? b.defaults() : b.defaults, f = CKEDITOR.dom.element.createFromHtml(b.template.output(f)), g, j = a.widgets.wrapElement(f, b.name), i = new CKEDITOR.dom.documentFragment(j.getDocument());
                    i.append(j);
                    (g = a.widgets.initOn(f, b, d && d.startupData)) ? (f = g.once("edit", function (b) {
                        if (b.data.dialog)g.once("dialog",
                            function (b) {
                                var b = b.data, d, f;
    ...
    function E(a) {
        ...
        if ("wysiwyg" == this.editor.mode) {
            ...
            if (b) {
                for (d in c)b.contains(c[d].wrapper) ||
                this.destroy(c[d], !0);
                if (a && a.initOnlyNew)c = this.initOnAll(); else {
                    var j = b.find(".cke_widget_wrapper"), c = [];
                    d = 0;
                    for (e = j.count(); d < e; d++) {
                        f = j.getItem(d);
                        if (g = !this.getByElement(f, !0)) {
                            a:{
                                g = N;
                                for (var i = f; i = i.getParent();)if (g(i)) {
                                    g = !0;
                                    break a
                                }
                                g = !1
        ...
        return {
            toBeWrapped: b, iterator: function (a) {
                ...
                if (l = c.length) {
                    ...
                    for (i = 0; i < l; ++i)if (f = c[i], j = {}, g = f[0](a,
                            j))return g instanceof CKEDITOR.htmlParser.element && (a = g), a.attributes["data-cke-widget-data"] = encodeURIComponent(JSON.stringify(j)), a.attributes["data-cke-widget-upcasted"] = 1, b.push([a, f[1]]), !1
                }
            }
        }
    }
    ...
    function m(a, b, c, d) {
        var e = a.editor;
        e.fire("lockSnapshot");
        c ? (d = c.data("cke-widget-editable"), d = b.editables[d], a.widgetHoldingFocusedEditable = b, b.focusedEditable = d, c.addClass("cke_widget_editable_focused"), d.filter && e.setActiveFilter(d.filter), e.setActiveEnterMode(d.enterMode,
            d.shiftEnterMode)) : (d || b.focusedEditable.removeClass("cke_widget_editable_focused"), b.focusedEditable = null, a.widgetHoldingFocusedEditable = null, e.setActiveFilter(null), e.setActiveEnterMode(null, null));
    ...
    function I(a) {
        ...
        b.on("drop", function (c) {
            var e = c.data.dataTransfer, f = e.getData("cke/widget-id"), g = b.createRange();
            if (!("" === f || e.getTransferType(b) != CKEDITOR.DATA_TRANSFER_INTERNAL))if (e = a.instances[f])g.setStartBefore(e.wrapper), g.setEndAfter(e.wrapper), c.data.dragRange = g, delete CKEDITOR.plugins.clipboard.dragStartContainerChildCount, delete CKEDITOR.plugins.clipboard.dragEndContainerChildCount, c.data.dataTransfer.setData("text/html",
                b.editable().getHtmlFromRange(g).getHtml()), b.widgets.destroy(e, !0)
        });
        b.on("contentDom", function () {
            ...
            CKEDITOR.tools.extend(a, {
                finder: new c.finder(b, {
                    lookups: {
                        "default": function (b) {
                            if (!b.is(CKEDITOR.dtd.$listItem) && b.is(CKEDITOR.dtd.$block) && !h.isDomNestedEditable(b)) {
    ...
    function G(a) {
        ...
        b.on("contentDom", function () {
            ...
            c.attachListener(d, "mousedown", function (b) {
                ...
                e = a.getByElement(c);
                f = 0;
                e && (e.inline && c.type == CKEDITOR.NODE_ELEMENT && c.hasAttribute("data-cke-widget-drag-handler") ?
                    f = 1 : h.getNestedEditable(e.wrapper, c) ? e = null : (b.data.preventDefault(), CKEDITOR.env.ie || e.focus()))
            });
    ...
    function F(a) {
        ...
        b.on("selectionChange", function (c) {
            var d = (c = h.getNestedEditable(b.editable(), c.data.selection.getStartElement())) && a.getByElement(c), e = a.widgetHoldingFocusedEditable;
            if (e) {
                if (e !==
                    d || !e.focusedEditable.equals(c))m(a, e, null), d && c && m(a, d, c)
            } else d && c && m(a, d, c)
        });
    ...
    function D(a) {
        ...
        b.on("toDataFormat", function (a) {
            if (a.data.downcastingSessionId)for (var a = c[a.data.downcastingSessionId], b, f, g, j, h, l; b = a.shift();) {
                f = b.widget;
                g = b.element;
                j = f._.downcastFn && f._.downcastFn.call(f,
                        g);
                for (l in b.editables)h = b.editables[l], delete h.attributes.contenteditable, h.setHtml(f.editables[l].getData());
                j || (j = g);
    ...
    function C(a) {
        ...
        b.on("dataReady", function () {
            if (d)for (var c = b.editable().find(".cke_widget_wrapper"), f, g, j = 0, i = c.count(); j < i; ++j)f = c.getItem(j), g = f.getFirst(h.isDomWidgetElement), g.type == CKEDITOR.NODE_ELEMENT && g.data("widget") ? (g.replace(f), a.wrapElement(g)) : f.remove();
            d = 0;
            a.destroyAll(!0);
            a.initOnAll()
        });
        ...
        b.on("paste", function (a) {
            ...
            if (a.range && (a = h.getNestedEditable(b.editable(), a.range.startContainer)))(a = CKEDITOR.filter.instances[a.data("cke-filter")]) && b.setActiveFilter(a)
        });
    ...
    function w(a) {
        ...
        return {
            select: function (a) {
                ...
                var f = a.focused !== e, g, h;
                a.editor.fire("lockSnapshot");
                for (f && (g = a.focused) && q(a, g); g = d.pop();)b.splice(CKEDITOR.tools.indexOf(b, g), 1), g.isInited() && (h = g.editor.checkDirty(), g.setSelected(!1), !h && g.editor.resetDirty());
                f && e && (h = a.editor.checkDirty(), a.focused = e, a.fire("widgetFocused",
                    {widget: e}), e.setFocused(!0), !h && a.editor.resetDirty());
    ...
    function x(a, b, c) {
        var d = 0, b = y(b), e = a.data.classes || {}, f;
        if (b) {
    ...
    function v(a, b) {
        ...
        if (!d.getById("cke_copybin")) {
            var e = c.blockless || CKEDITOR.env.ie ? "span" : "div", f = d.createElement(e), g = d.createElement(e), e = CKEDITOR.env.ie &&
            ...
            f.setHtml('<span data-cke-copybin-start="1">​</span>' + c.editable().getHtmlFromRange(h).getHtml() + '<span data-cke-copybin-end="1">​</span>');
            c.fire("saveSnapshot");
            c.fire("lockSnapshot");
            g.append(f);
            c.editable().append(g);
            var i = c.on("selectionChange", z, null, null, 0), l = a.repository.on("checkSelection", z, null, null, 0);
            if (e)var k = d.getDocumentElement().$, m = k.scrollTop;
            h = c.createRange();
    ...
    function A() {
        var a =
            CKEDITOR.document.getActive(), b = this.editor, c = b.editable();
    ...
    function T(a) {
        if (a.draggable) {
            var b = a.editor,
                c = a.wrapper.getLast(h.isDomDragHandlerContainer), d;
            ...
            if (!a.inline && (d.on("mousedown", U, a), CKEDITOR.env.ie && 9 > CKEDITOR.env.version))d.on("dragstart", function (a) {
    ...
    function U(a) {
        function b() {
            ...
            CKEDITOR.tools.isEmpty(e.visible) || (c = d.getRange(c[0]), this.focus(), f.getSelection().reset(), f.fire("drop", {
                dropRange: c,
        ...
        var c = this.repository.finder, d = this.repository.locator, e = this.repository.liner, f = this.editor, g = f.editable(), h = [], i = [];
        this.repository._.draggedWidget = this;
        var l =
            c.greedySearch(), k = CKEDITOR.tools.eventsBuffer(50, function () {
            m = d.locate(l);
            i = d.sort(n, 1);
    ...
    n.prototype = {
        ...
        checkSelection: function () {
            var a = this.editor.getSelection(), b = a.getSelectedElement(), c = w(this), d;
            ...
            if (this.focused === a) {
                ...
                if (!(d =
                        c.moveToClosestEditablePosition(a.wrapper, !0)))d = c.moveToClosestEditablePosition(a.wrapper, !1);
            ...
            if (b && !a) {
                ...
                e[d], this.destroy(c, a)
        }, finalizeCreation: function (a) {
            if ((a = a.getFirst()) && h.isDomWidgetWrapper(a))this.editor.insertElement(a), a = this.getByElement(a), a.ready = !0, a.fire("ready"), a.focus()
            ...
            for (var a = (a || this.editor.editable()).find(".cke_widget_new"), b = [], c, d = a.count(); d--;)(c = this.initOn(a.getItem(d).getFirst(h.isDomWidgetElement))) && b.push(c);
            return b
        }, onWidget: function (a) {
            ...
            if (!a)return null;
            for (var a = CKEDITOR.tools.trim(a).split(/\s+/), b, c = {}, d = 0; b = a.pop();)-1 == b.indexOf("cke_") && (c[b] = d = 1);
            return d ? c : null
            ...
            if (a instanceof CKEDITOR.dom.element) {
                ...
                if (!d)return null;
                if ((c = a.getParent()) && c.type ==
                    CKEDITOR.NODE_ELEMENT && c.data("cke-widget-wrapper"))return c;
                ...
                if (!d)return null;
                if ((c = a.parent) &&
                    c.type == CKEDITOR.NODE_ELEMENT && c.attributes["data-cke-widget-wrapper"])return c;
    ...
    h.prototype = {
        ...
        hasClass: function (a) {
            ...
            return c && c.is(CKEDITOR.dtd.$editable) ? (c = new o(this.editor, c, {filter: r.call(this.repository, this.name, a, b)}), this.editables[a] = c, c.setAttributes({
                ...
                b.pathName), this.editor.focusManager.add(c), c.on("focus", B, this), CKEDITOR.env.ie && c.on("blur", A, this), c._.initialSetData = !0, c.setData(c.getHtml()), !0) : !1
        }, _findOneNotNested: function (a) {
            for (var a = this.wrapper.find(a), b, c, d = 0; d < a.count(); d++)if (b = a.getItem(d), c = b.getAscendant(h.isDomWidgetWrapper), this.wrapper.equals(c))return b;
            return null
        }, isInited: function () {
    ...
    var R = RegExp('^(?:<(?:div|span)(?: data-cke-temp="1")?(?: id="cke_copybin")?(?: data-cke-temp="1")?>)?(?:<(?:div|span)(?: style="[^"]+")?>)?<span [^>]*data-cke-copybin-start="1"[^>]*>.?</span>([\\s\\S]+)<span [^>]*data-cke-copybin-end="1"[^>]*>.?</span>(?:</(?:div|span)>)?(?:</(?:div|span)>)?$'),
    ...
    (function () {
        ...
        CKEDITOR.style.addCustomHandler({
            ...
            checkActive: function (a, b) {
                ...
                return !h.isDomWidgetWrapper(a) ? !1 : (a = a.getFirst(h.isDomWidgetElement)) && a.data("widget") == this.widget
